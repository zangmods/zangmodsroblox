...
function Library:CreateWindow(WindowInfo)
    ...
    -- MODIFICAÇÃO: Variáveis para controlar a aba expandida e o timer de auto-recolhimento
    local ExpandedTab = nil
    local CollapseTimer = nil
    local CollapseDelay = 3 -- Tempo em segundos para a aba voltar ao normal
    ...
    function Window:AddTab(...)
        ...
        local TabIcon
        local TabNameLabel -- MODIFICAÇÃO: Variável para o nome da aba
        ...
        do
            ...
            -- MODIFICAÇÃO: Layout para organizar ícone e texto
            New("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                VerticalAlignment = Enum.VerticalAlignment.Center,
                SortOrder = Enum.SortOrder.LayoutOrder,
            ...
            if Icon then
                TabIcon = New("ImageLabel", {
                    ...
                    ImageTransparency = 0.5,
                    Size = UDim2.new(0, 32, 1, -16),
                    Parent = TabButton,
                })
            end

            -- MODIFICAÇÃO: Label para o nome da aba
        ...
        local Tab = {
            ...
            NameLabel = TabNameLabel, -- MODIFICAÇÃO: Referência para a label de nome
            IsExpanded = false, -- MODIFICAÇÃO: Estado de expansão
            Groupboxes = {},
            Tabboxes = {},
        ...
        -- MODIFICAÇÃO: Nova função para encolher a aba
        function Tab:Collapse(instant)
            if not Tab.IsExpanded then return end
            Tab.IsExpanded = false

            local tween = instant and TweenInfo.new(0) or Library.TweenInfo

            TweenService:Create(TabButton, tween, { Size = UDim2.new(0, 48, 0, 48) }):Play()
            TweenService:Create(Tab.NameLabel, tween, { TextTransparency = 1 }):Play()

            if ExpandedTab == Tab then
                ExpandedTab = nil
            end
        end
        ...
        function Tab:Show()
            ...
            -- MODIFICAÇÃO: Lógica de expansão e timer
            if CollapseTimer then
                task.cancel(CollapseTimer)
                CollapseTimer = nil
            end
            if ExpandedTab and ExpandedTab ~= Tab then
                ExpandedTab:Collapse(true)
            end
            ...
            if not Tab.IsExpanded then
                Tab.IsExpanded = true
                local textBounds = Library:GetTextBounds(Tab.NameLabel.Text, Tab.NameLabel.FontFace, Tab.NameLabel.TextSize)
                local newWidth = 32 + 20 + textBounds.X -- Largura do ícone + paddings + largura do texto
                
                TweenService:Create(TabButton, Library.TweenInfo, { Size = UDim2.new(0, newWidth, 0, 48) }):Play()
            ...
            CollapseTimer = task.delay(CollapseDelay, function()
                if ExpandedTab == Tab then
                    Tab:Collapse()
                end
            end)
        ...
        function Tab:Hide()
            Tab:Collapse(true) -- MODIFICAÇÃO: Encolhe a aba instantaneamente ao trocar

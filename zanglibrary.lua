...
function Library:CreateWindow(WindowInfo)
    WindowInfo = Library:Validate(WindowInfo, Templates.Window)
        -- ... logo após a validação de WindowInfo
    
    local hideTabsTimer = nil -- Para controlar o temporizador de esconder as abas
    local tabNameWidths = {} -- Para guardar a largura dos nomes das abas
    ...
    do
        ...
        local function AnimateTabs(show)
        -- Esta função é chamada em uma nova thread para não bloquear o jogo
        task.spawn(function()
            local baseTabWidth = 48 -- Largura base da aba (só com ícone)
            local namePadding = 12 -- Espaçamento extra para o nome
            local layoutPadding = Tabs.UIListLayout.Padding.Offset -- Espaçamento entre as abas

            local totalWidth = 0

            -- Itera por todas as abas para animar cada uma
            for _, tabInfo in pairs(Library.Tabs) do
                if not tabInfo.IsKeyTab and tabInfo.Button then
                    local tabButton = tabInfo.Button
                    local nameLabel = tabInfo.NameLabel
                    local nameWidth = tabNameWidths[tabButton] or 0

                    -- Calcula a largura final do botão e do nome
                    local targetButtonWidth = show and (baseTabWidth + nameWidth + namePadding) or baseTabWidth
                    local targetNameWidth = show and nameWidth or 0
                    
                    totalWidth = totalWidth + targetButtonWidth + layoutPadding

                    -- Cria as animações (Tweens)
                    TweenService:Create(tabButton, Library.TweenInfo, { Size = UDim2.new(0, targetButtonWidth, 0, 48) }):Play()
                    TweenService:Create(nameLabel, Library.TweenInfo, { Size = UDim2.new(0, targetNameWidth, 1, 0) }):Play()
                end
            end
            
            totalWidth = totalWidth - layoutPadding -- Remove o padding do último item

            -- Anima a largura da barra de abas e reposiciona o container principal
            local targetTabsFrameWidth = math.max(60, totalWidth + 12) -- +12 de padding final
            
            TweenService:Create(Tabs, Library.TweenInfo, { Size = UDim2.new(0, targetTabsFrameWidth, 1, -70)}):Play()
            TweenService:Create(Container, Library.TweenInfo, { 
                Position = UDim2.new(0, targetTabsFrameWidth + 1, 0, 49),
                Size = UDim2.new(1, -(targetTabsFrameWidth + 2), 1, -70)
            }):Play()
        end)
    end
    ...
    local Window = {}
        ...
        local TabNameLabel -- NOVO: Label para o nome
        ...
        do
            ...
            -- Adiciona um layout horizontal dentro do botão da aba
            New("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                VerticalAlignment = Enum.VerticalAlignment.Center,
                SortOrder = Enum.SortOrder.LayoutOrder,
            ...
            if Icon then
                TabIcon = New("ImageLabel", {
                    LayoutOrder = 1, -- Ordem no layout
                    Image = Icon.Url,
                    ImageColor3 = Icon.Custom and "White" or "AccentColor",
                    ImageRectOffset = Icon.ImageRectOffset,
                    ImageRectSize = Icon.ImageRectSize,
                    ImageTransparency = 0.5,
                    Size = UDim2.fromOffset(32, 32), -- Tamanho fixo do ícone
                    BackgroundTransparency = 1,
                    Parent = TabButton,
                })
            end

            -- NOVO: Cria o TextLabel para o nome, inicialmente invisível
            TabNameLabel = New("TextLabel", {
                LayoutOrder = 2, -- Aparece depois do ícone
                BackgroundTransparency = 1,
                Size = UDim2.new(0, 0, 1, 0), -- Largura inicial zero
                Text = Name,
                TextSize = 14,
            ...
            Library:AddToRegistry(TabNameLabel, {TextColor3 = "FontColor"})

            -- Calcula e armazena a largura do texto para a animação
            local nameWidth, _ = Library:GetTextBounds(Name, TabNameLabel.FontFace, TabNameLabel.TextSize)
            tabNameWidths[TabButton] = nameWidth
        ...
        local Tab = {
            ...
            Button = TabButton, -- NOVO
            NameLabel = TabNameLabel, -- NOVO
            Groupboxes = {},
            Tabboxes = {},
        ...
        -- AQUI COMEÇAM AS MUDANÇAS IMPORTANTES
        function Tab:Show()
            if Library.ActiveTab then
                Library.ActiveTab:Hide()
            end
            
            -- Cancela o timer anterior e inicia a animação
            if hideTabsTimer then
                task.cancel(hideTabsTimer)
                hideTabsTimer = nil
            end
            AnimateTabs(true) -- Mostra os nomes
            
            -- Agenda para esconder os nomes após 5 segundos
            hideTabsTimer = task.delay(5, function()
                if Library.ActiveTab then -- Só esconde se uma aba estiver ativa
                    AnimateTabs(false)
                end
                hideTabsTimer = nil
            end)
        ...
        if not Library.ActiveTab then
            Tab:Show()
            -- Esconde os nomes na primeira vez que a UI abre
    ...
    Library:AddDraggableButton("ZangMods", function()
        Library:Toggle()
    end)
